defmodule Gamora.Cache.Actions do
  @moduledoc """
  ...
  defmacro __using__(_args) do
    quote do
      ...
      def get(access_token) do
        data = adapter().get(key(access_token))
        {:ok, data}
      end
      ...
      def put(access_token, data) do
        adapter().put(key(access_token), data, ttl: expires_in())
        {:ok, data}
      end
      ...
      def del(access_token) do
        adapter().delete(key(access_token))
        {:ok, true}
      end

      defp adapter do
        Application.get_env(:ueberauth, AuthenticatedUser, [])
        |> Keyword.get(:cache_adapter, Gamora.Cache)
      end
    end
  end
end
